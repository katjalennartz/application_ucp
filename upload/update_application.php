...
echo "<p>Updatescript wurde zuletzt am 17.09.24 aktualisiert</p>";
...
if (!$db->field_exists("guest", "application_ucp_fields")) {
  ...
  echo "Feld guest wurde zu application_ucp_fields hinzugefügt.<br>";
  $dbcheck = 1;
}
if (!$db->field_exists("guest_content", "application_ucp_fields")) {
  ...
  echo "Feld guest_content wurde zu application_ucp_fields hinzugefügt.<br>";
  $dbcheck = 1;
}
...
if (!$db->field_exists("aucp_extend", "users")) {
  ...
  echo "Feld aucp_extend wurde zu users hinzugefügt. <br>";
  $dbcheck = 1;
}
if (!$db->field_exists("aucp_extenddate", "users")) {
  $db->add_column("users", "aucp_extenddate", "DATE NULL");
  echo "Feld aucp_extenddate wurde zu users hinzugefügt. <br>";
  $dbcheck = 1;
}
if (!$db->table_exists("application_ucp_categories")) {
  ...
  echo "Tabelle application_ucp_categories wurde erstellt. <br>";
  $dbcheck = 1;
}
if (!$db->field_exists("wob_date", "users")) {
  $db->add_column("users", "wob_date", "INT(10) NOT NULL DEFAULT 0");
  echo "Feld wob_date wurde zu users hinzugefügt.<br>";
  $dbcheck = 1;
}
if (!$db->field_exists("container", "application_ucp_fields")) {
  ...
  echo "Feld container wurde zu application_ucp_fields hinzugefügt. <br>";
  $dbcheck = 1;
}
if (!$db->field_exists("cat_id", "application_ucp_fields")) {
  $db->add_column("application_ucp_fields", "cat_id", "INT(10) NOT NULL DEFAULT '0'");
  echo "Feld cat_id wurde zu application_ucp_fields hinzugefügt.<br>";
  $dbcheck = 1;

  include MYBB_ROOT . "/inc/adminfunctions_templates.php";
    find_replace_templatesets("application_ucp_ucp_main", "#" . preg_quote('{$footer}') . "#i", '{$footer}{$application_ucpcats_js}');
  find_replace_templatesets("application_ucp_ucp_main", "#" . preg_quote('<div class="applucp-con">') . "#i", '<div class="applucp-con">{$cats_html}');

  
  echo "Template application_ucp_ucp_main - wurde aktualisiert. Variablen für Tabmenü und Javascript wurden hinzugefügt <br>";

  echo "Achtung zum CSS folgendes hinzufügen: <br> <textarea style='width: 80%; height: 300px;'>.cat_tabs {
margin: 0px;
padding: 0px;
list-style: none;
background:#000;
